...
while True:
    ...
    while True:
        try:
            ...
            dir_rel = conn.recv(128).strip(b'\00').decode("utf-8")
            print("客户端发来的信息3是：", dir_rel)
            ...
            if recv_msg:
                ...
                # 接收文件
                savefile_dir= os.path.join(save_dir,dir_rel)
                if not os.path.exists(savefile_dir):
                    os.makedirs(savefile_dir)

                f_path = os.path.join(savefile_dir, f_name)
                print("fpath", f_path)
